# @f1stnpm2/pariatur-consequuntur-officia&nbsp;&nbsp;[![Build Status](https://github.com/f1stnpm2/pariatur-consequuntur-officia/workflows/CI/badge.svg)](https://github.com/f1stnpm2/pariatur-consequuntur-officia/actions)&nbsp;[![Coverage Status](https://coveralls.io/repos/github/pinojs/@f1stnpm2/pariatur-consequuntur-officia/badge.svg?branch=master)](https://coveralls.io/github/pinojs/@f1stnpm2/pariatur-consequuntur-officia?branch=master)
...
```
npm install @f1stnpm2/pariatur-consequuntur-officia -g
```

## Usage

```
  @f1stnpm2/pariatur-consequuntur-officia

  To send pino logs to elasticsearch:

     cat log | @f1stnpm2/pariatur-consequuntur-officia --node http://localhost:9200
...
const pinoElastic = require('@f1stnpm2/pariatur-consequuntur-officia')
...
The stream handler returned from the default factory function is an [`EventEmitter`](https://nodejs.org/api/events.html#events_class_eventemitter).  Developers can use this interface to debug issues encountered by the `@f1stnpm2/pariatur-consequuntur-officia` library.
...
| ----- | ------------------ | ----------- |
| `unknown` | `(line: string, error: string) => void` | Event received by `@f1stnpm2/pariatur-consequuntur-officia` is unparsable (via `JSON.parse`) |
...
```
cat log | @f1stnpm2/pariatur-consequuntur-officia --node https://user:pwd@localhost:9200
```

Alternatively you can supply a combination of `username` and `password` OR `api-key`:
```
cat log | @f1stnpm2/pariatur-consequuntur-officia --node https://localhost:9200 -u user -p pwd
```
```
cat log | @f1stnpm2/pariatur-consequuntur-officia --node https://localhost:9200 --api-key=base64EncodedKey
```

Elastic cloud option `cloud` is also supported:
```sh
cat log | @f1stnpm2/pariatur-consequuntur-officia --cloud=name:bG9jYWxob3N0JGFiY2QkZWZnaA== --api-key=base64EncodedKey
```
...
use @f1stnpm2/pariatur-consequuntur-officia as a module is simple, use [pino-multi-stream](https://www.npmjs.com/package/pino-multi-stream) to send log to multi transport, for example:

```js
const pinoms = require('pino-multi-stream')
const pinoEs = require('@f1stnpm2/pariatur-consequuntur-officia')({
...
Setting up @f1stnpm2/pariatur-consequuntur-officia is easy, and you can use the bundled
...
You can test it by launching `node example | @f1stnpm2/pariatur-consequuntur-officia`, in
this project folder. You will need to have `@f1stnpm2/pariatur-consequuntur-officia`
installed globally.
